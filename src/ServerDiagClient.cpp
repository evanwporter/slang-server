...
void ServerDiagClient::report(const slang::ReportedDiagnostic& diag) {
    ...
    auto guessTokenLenFromMessage = [](std::string_view msg) -> size_t {
        auto l = msg.find('\'');
    ...
    auto getLocation = [&](SourceLocation loc, std::span<const SourceRange> ranges,
        ...
        if (ranges.empty()) {
            if (hasLocation) {
                size_t len = guessTokenLenFromMessage(message);
                return lsp::Location{
                    .uri = URI::fromFile(m_sourceManager.getFullPath(loc.buffer())),
                    .range = toRange(loc, m_sourceManager, len),
                };
            }
            else {
